  ...
  console.log('siteURL:', siteUrl);
    ...
    try {
      const browser = await puppeteer.launch({ headless: false, executablePath: '/usr/bin/google-chrome', args: ['--no-sandbox', '--disable-gpu','--disable-setuid-sandbox', ] });
    ...
    // Save page HTML and screenshot for inspection
      ...
      await fs.writeFileSync('pageContent.html', pageContent  );
